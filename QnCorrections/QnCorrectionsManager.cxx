...
void QnCorrectionsManager::RotateQvec(QnCorrectionsConfiguration* QnConf) {
  ...
  Int_t bin = fInputHistograms[iconf]->GetRotationHistogramE(0)->GetBin(fillValues);
    ...
    for(Int_t ih=QnConf->MinimumHarmonic(); ih<=QnConf->MaximumHarmonic(); ++ih) {QvectorRotated->SetQnVectorStatus(ih, QnCorrectionsConstants::kAlignment);}
...
//
//    QnCorrectionsHistograms* histos;
//    histos = new QnCorrectionsHistograms();
//
//  THashList* dataByRun = new THashList();
//  //THashList* dataByRun = fListOutputHistogramsQnCorrections;//new THashList();
//  dataByRun->SetName(runlabel);
