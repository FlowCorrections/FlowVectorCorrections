...
ClassImp(QnCorrectionsSteps)


//_______________________________________________________________________________
QnCorrectionsSteps::QnCorrectionsSteps() 
{
  //
  // default constructor
  //
...
//void QnCorrectionsSteps::BuildQnVectors(QnCorrectionsQnVector* QvectorOut, TClonesArray* dataVectorArray, Int_t QnConfIndex, Int_t minHar, Int_t maxHar, Int_t EqualizationMethod){
//  //
//  // Construct the event plane for a specified detector
//  //
//
//  if(EqualizationMethod==-1) QnCorrectionsDataVector::FillQvector(dataVectorArray,QnConfIndex, QvectorOut);
//  else                       QnCorrectionsDataVector::FillQvector(dataVectorArray,QnConfIndex, QvectorOut, EqualizationMethod);
//
//
//  if(QvectorOut->N()==0) {               // If detector is empty
//    for(Int_t ih=minHar; ih<=maxHar; ++ih) QvectorOut->SetQnVectorStatus(ih, QnCorrectionsQnVector::kUndefined);
//    return;
//  }
//  else{
//    for(Int_t ih=minHar; ih<=maxHar; ++ih){
//      if(EqualizationMethod==-1) QvectorOut->SetQnVectorStatus(ih, QnCorrectionsQnVector::kRaw);
//      else                       QvectorOut->SetQnVectorStatus(ih, QnCorrectionsQnVector::kEqualized);
//    }
//  }
//
//  return;
...
void QnCorrectionsSteps::CalibrateDataVector(TClonesArray* dataVectorArray, QnCorrectionsConfiguration* QnConf,  QnCorrectionsHistograms* inputHistos, Double_t* fillValues) {
  //
...
//_____________________________________________________________________
void QnCorrectionsSteps::RecenterQvec(QnCorrectionsQnVector* QvectorIn, QnCorrectionsQnVector* QvectorOut, QnCorrectionsHistograms* inputHistos, Int_t bin, Int_t useStep, Int_t minHar, Int_t maxHar) {
  ...
  //
      ...
      for(Int_t ih=minHar; ih<= maxHar; ++ih) {
        ...
        if(QvectorIn->CheckQnVectorStatus(ih,QnCorrectionsConstants::kUndefined)) continue;
        ...
        QvectorOut->SetQnVectorStatus(ih, QnCorrectionsConstants::kRecentering);
        if(axentries==1) QvectorOut->SetQnVectorStatus(ih, QnCorrectionsConstants::kUndefined);   // With one event in the event class, Qvector becomes undefined after recentering
...
//_____________________________________________________________________
void QnCorrectionsSteps::TwistAndRescale2nQn(QnCorrectionsQnVector* QvectorIn, QnCorrectionsQnVector* QvectorTwist, QnCorrectionsQnVector* QvectorRescale, QnCorrectionsHistograms* inputHistos, Int_t bin, Int_t minHar, Int_t maxHar, Bool_t doTwist, Bool_t doRescaling){
  ...
  for(Int_t ih=minHar; ih<=maxHar; ++ih) {
    ...
    if(QvectorIn->CheckQnVectorStatus(ih, QnCorrectionsConstants::kUndefined)) continue;
      ...
      if(doTwist){
        ...
        QvectorTwist->SetQnVectorStatus(ih, QnCorrectionsConstants::kTwist);
        QvectorRescale->SetQnVectorStatus(ih, QnCorrectionsConstants::kTwist);
      }
      ...
      if(doRescaling){
        ...
        QvectorRescale->SetQnVectorStatus(ih, QnCorrectionsConstants::kRescaling);
      }
...
//_____________________________________________________________________
void QnCorrectionsSteps::TwistAndRescale3DetectorCorrelation(QnCorrectionsQnVector* QvectorIn, QnCorrectionsQnVector* QvectorTwist, QnCorrectionsQnVector* QvectorRescale, QnCorrectionsHistograms* inputHistos, Int_t bin, Int_t minHar, Int_t maxHar, Bool_t doTwist, Bool_t doRescaling, Int_t eventClassParameter){
...
////_____________________________________________________________________
//void QnCorrectionsSteps::TwoDetectorCorrelationTwistQvec(Float_t* values, Int_t corpar) {
//
//  //
//  // Recenter the detector event plane
//  //
...
//        if(!qVector) continue;
//
//	      if(!qVector->CheckQnVectorStatus(ih, QnCorrectionsQnVector::kRecentered)) continue;
//      
//        Qx = qVector->Qx(ih);
//        Qy = qVector->Qy(ih);
//
...
//
//	      qVector->SetQnVectorStatus(ih, QnCorrectionsQnVector::kDiagonalized);
//      }
//    }
//  }
//  }
...
//
////_____________________________________________________________________
//void QnCorrectionsSteps::CorrelationRescalingQvec(Float_t* values, Int_t corpar) {
//
//  //
//  // Recenter the detector event plane
//  //
...
//        if(!qVector) continue;
//
//	      if(!qVector->CheckQnVectorStatus(ih, QnCorrectionsQnVector::kDiagonalized)) continue;
//      
//        Qx = qVector->Qx(ih);
//        Qy = qVector->Qy(ih);
//
...
//
//	      qVector->SetQnVectorStatus(ih, QnCorrectionsQnVector::kRescaled);
//      }
//    }
//  }
//  }
...
//
////_____________________________________________________________________
//void QnCorrectionsSteps::CorrelationTwistAndRescalingQvec(Float_t* values, Int_t corpar) {
//
//  //
//  // Recenter the detector event plane
//  //
...
//
////_____________________________________________________________________
//void QnCorrectionsSteps::ThreeDetectorCorrelationTPCTwistQvec(Float_t* values, Int_t corpar) {
//
//  //
//  // Recenter the detector event plane
//  //
...
//          if(!qVector->CheckQnVectorStatus(ih, QnCorrectionsQnVector::kEqualized)) continue;}
//        else if(!qVector->CheckQnVectorStatus(ih, QnCorrectionsQnVector::kRecentered)) continue;
//
//        //if(!qVector->CheckQnVectorStatus(ih, QnCorrectionsQnVector::kRecentered)) continue;
//      
//        Qx = qVector->Qx(ih);
...
//
////_____________________________________________________________________
//void QnCorrectionsSteps::ThreeDetectorCorrelationTPCRescalingQvec(Float_t* values, Int_t corpar) {
//
//  //
//  // Recenter the detector event plane
//  //
...
//        qVector=static_cast<QnCorrectionsQnVector*>(qvecList->At(ibin));
//
//        //cout<<"hello  "<<qVector->CheckQnVectorStatus(ih, QnCorrectionsQnVector::kEqualized)<<"  "<<qVector->CheckQnVectorStatus(ih, QnCorrectionsQnVector::kUndefined)<<endl;
//	      if(!qVector->CheckQnVectorStatus(ih, QnCorrectionsQnVector::kDiagonalized)) continue;
//      
//        //cout<<"hey  "<<qVector->CheckQnVectorStatus(ih, QnCorrectionsQnVector::kEqualized)<<"  "<<qVector->CheckQnVectorStatus(ih, QnCorrectionsQnVector::kUndefined)<<endl;
//        Qx = qVector->Qx(ih);
//        Qy = qVector->Qy(ih);
//
//        QxRescaled = Qx / Ap;
...
//     	  qVector->SetQy( ih, QyRescaled);
//
//        //cout<<"hey 2  "<<qVector->CheckQnVectorStatus(ih, QnCorrectionsQnVector::kRescaled)<<endl;
//	      qVector->SetQnVectorStatus(ih, QnCorrectionsQnVector::kRescaled);
//        //cout<<"hey 3  "<<qVector->CheckQnVectorStatus(ih, QnCorrectionsQnVector::kRescaled)<<endl;
//      }
//    }
//  }
// }
...
//
////_____________________________________________________________________
//void QnCorrectionsSteps::ThreeDetectorCorrelationTPCTwistAndRescalingQvec(Float_t* values, Int_t corpar) {
//
//  //
//  // Recenter the detector event plane
//  //
...
//_____________________________________________________________________
//void QnCorrectionsSteps::TwistQnVector() {
...
//_____________________________________________________________________
//void QnCorrectionsSteps::RescaleQnVector(Int_t u2npar) {
...
//
////_____________________________________________________________________
//void QnCorrectionsSteps::U2nTwistAndRescalingQvec(Float_t* values, Int_t u2npar) {
//
//  //
//  // Recenter the detector event plane
//  //
...
//
////_____________________________________________________________________
//void QnCorrectionsSteps::RotateQvec() {
//
//  //
//  // Recenter the detector event plane
//  //
...
//
//        if(!qVector->CheckQnVectorStatus(ih, QnCorrectionsQnVector::kRecentered)) continue;
//        if(qVector->CheckQnVectorStatus(ih, QnCorrectionsQnVector::kAligned)) continue;
//
//
//        Qx = qVector->Qx(ih);
//        Qy = qVector->Qy(ih);
...
//
//        qVector->SetQnVectorStatus(ih, QnCorrectionsQnVector::kAligned);
//      }
//    }
//  }
// }
