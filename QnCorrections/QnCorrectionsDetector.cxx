...
/// \param list list where the histograms should be incorporated for its persistence
/// \return kTRUE if everything went OK
Bool_t QnCorrectionsDetector::CreateNveQAHistograms(TList *list) {
  Bool_t retValue = kTRUE;
  for (Int_t ixConfiguration = 0; ixConfiguration < fConfigurations.GetEntriesFast(); ixConfiguration++) {
...
///
/// The request is transmitted to the attached detector configurations
void QnCorrectionsDetector::AfterInputsAttachActions() {
...
void QnCorrectionsDetector::AttachCorrectionsManager(QnCorrectionsManager *manager) {
