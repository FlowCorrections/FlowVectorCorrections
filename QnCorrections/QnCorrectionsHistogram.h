...
/// \date Jun 16, 2016
class QnCorrectionsHistogram : public QnCorrectionsHistogramBase {
public:
  QnCorrectionsHistogram();
  QnCorrectionsHistogram(const char *name,
      const char *title,
      QnCorrectionsEventClassVariablesSet &ecvs);
  virtual ~QnCorrectionsHistogram();
...
private:
  ...
  /// \cond CLASSIMP
  ClassDef(QnCorrectionsHistogram, 1);
  /// \endcond
};
